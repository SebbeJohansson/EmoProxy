...
func writeProxyAPIHeaders(w http.ResponseWriter) {
...
func makeApiRequest(r *http.Request) string {
	...
	// TODO: only redirect to override logic if enabled
	override := overrideApiRequest(body)
	if override != "" {
		saveRequest(r.URL.RequestURI(), string(requestBody), override)
		return override
	}
